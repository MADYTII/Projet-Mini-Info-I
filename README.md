...
- `make` : Compile tous les fichiers et génère l'exécutable principal.
...
Une fois le projet compilé, vous pouvez exécuter le programme principal de trois manières :

1. Avec la commande `make run` :
   ```bash
   make run
   ```
   Cette commande compile et exécute directement le programme principal.

2. Avec la commande `make all` :
   ```bash
   make all
   ```
   Cette commande effectue la compilation, exécute le programme principal, puis nettoie automatiquement les fichiers temporaires.

3. En exécutant manuellement l'exécutable :
   ```bash
   ./programme_principal
   ```

Le programme chargera automatiquement les données des patients à partir du fichier `patients.pengu` et appliquera les algorithmes du perceptron pour la classification et l'évaluation des risques.
